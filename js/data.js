...
// ── Channel operations ───────────────────────────────────────
export async function addChannelToStack(channelData, categories, note) {
  const user = getLoggedInUser();
  if (!user) return { ok: false, error: 'Login required' };

  // Only call the channel API if we don't have stats yet (costs 1 unit vs search's 100)
  let details = channelData;
  if (!channelData.subscriberCount) {
    // Check if channel already exists in Convex first (free — no YouTube API call)
    const existing = await q(api.channels.getByYoutubeId, { youtubeChannelId: channelData.youtubeChannelId });
    if (existing) {
      details = existing;
    } else {
...
export async function addChannelToCollection(collectionId, channelId) {
  const user = getLoggedInUser();
  if (!user) return null;
  return await m(api.collections.addChannel, { id: collectionId, userId: user.id, channelId });
}

export async function removeChannelFromCollection(collectionId, channelId) {
  const user = getLoggedInUser();
  if (!user) return null;
  return await m(api.collections.removeChannel, { id: collectionId, userId: user.id, channelId });
}
